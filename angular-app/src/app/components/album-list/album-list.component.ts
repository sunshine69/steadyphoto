      ...
      @if (activeTab === 'sharedAlbums') {
        <div>
          ...
          @if (sharedAlbums && sharedAlbums.length > 0) {
            <div class="row">
              @for (album of sharedAlbums; track album) {
                <div class="col-md-4 col-lg-3 mb-4">
                  <div
                    class="card h-100 shadow-sm album-card"
                    ...
                    <div class="card-body text-center d-flex flex-column justify-content-center align-items-center py-5">
                      ...
                      @if (!album.description) {
                        <p class="card-text text-muted small">Shared album</p>
                      }
                    </div>
                  </div>
                </div>
    ...
    @if (showDeleteModal) {
      <div class="delete-modal-backdrop" (click)="closeDeleteModal()">
        <div class="delete-modal-content" (click)="$event.stopPropagation()">
          ...
          @if (selectedAlbums.length > 0) {
            <div class="bulk-delete-info">
              ...
              <ul class="list-group list-group-flush small">
                @for (a of selectedAlbums; track a) {
                  <li class="list-group-item bg-light">{{ a.name }}</li>
                }
              </ul>
            </div>
          }
          ...
          @if (albumToDelete && isBulkDelete) {
            <p class="fw-bold text-muted">Album: {{ albumToDelete.name }}</p>
          }
